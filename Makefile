...
.PHONY: help dump build build-all build-arcgispro extent validate lint clean

help:
	@echo "Targets:"
	@echo "  make dump DIR=project_dir          # finds .qgs or .qgz under DIR"
	@echo "  make build DIR=project_dir         # build .qgs or .aprx based on project type"
	@echo "  make build-all DIR=project_dir     # force rebuild even if up to date"
	@echo "  make build-arcgispro DIR=project_dir  # alias for make build (ArcGIS projects)"
...
build-arcgispro:
	@if [ -z "$(DIR)" ]; then echo "Usage: make build-arcgispro DIR=project_dir"; exit 1; fi
	uv run alidade-build $(DIR)
